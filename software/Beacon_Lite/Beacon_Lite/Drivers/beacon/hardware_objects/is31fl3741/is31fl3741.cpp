...
IS31FL3741::IS31FL3741()
  : i2c_(syscfg::i2c::bus::kIs31fl3741) {
  page_ = Page::k0;
...
HAL_StatusTypeDef IS31FL3741::initiate() {
    // HAL_StatusTypeDef status = i2c_.initiate();
    // if (status != HAL_OK) {return status;}

    // status = i2c_.action_add_device(reserved::IS31FL3741::kDeviceCfg,
    //                              kAddr);
    // if (status != HAL_OK) {return status;}
    HAL_StatusTypeDef status;

    status = action_reset_all_leds();
    if (status != HAL_OK) {return status;}

    status = setup_config_register();
    if (status != HAL_OK) {return status;}

    status = setup_pull_resistor();
    if (status != HAL_OK) {return status;}

    return status;
}

HAL_StatusTypeDef IS31FL3741::set_global_current(const uint8_t current) {
    ...
    return i2c_.act_pollVerifyWrite(kAddr, rGlobalCurrent, current);
}

HAL_StatusTypeDef IS31FL3741::set_specific_led(uint32_t led_num,
                                       const uint8_t pwm,
                                       const uint8_t scale) {
    ...
    if (led_num < kGroup1Max) {
        ...
        if (status != HAL_OK) {return status;}
        status = i2c_.act_pollVerifyWrite(kAddr, led_num, pwm);
        if (status != HAL_OK) {return status;}

        status = set_page(Page::k2);
        if (status != HAL_OK) {return status;}
        status = i2c_.act_pollVerifyWrite(kAddr, led_num, scale);
    } 
    else {
        led_num -= kGroup1Max;
...
HAL_StatusTypeDef IS31FL3741::set_rgb_led(LedConfig config) {
...
HAL_StatusTypeDef IS31FL3741::action_verify() {
    // constexpr uint8_t addr = reserved::i2c1::kAddr_IS31FL3741;
    // HAL_StatusTypeDef status = i2c_.action_probe(addr);
    // if (status != HAL_OK) {return status;}
    HAL_StatusTypeDef status;
    ...
    uint8_t kReadId;
    status = i2c_.act_pollRead(kAddr, rId, &kReadId);
    if (status != HAL_OK) {return status;}
...
HAL_StatusTypeDef IS31FL3741::action_off() {
    HAL_StatusTypeDef status = set_page(Page::k4);
    if (status != HAL_OK) {return status;}

    using namespace IS31FL3741_Config;
    status = i2c_.act_pollVerifyWrite(kAddr, rRegister, kDataOff);
    if (status != HAL_OK) {return status;}
...
HAL_StatusTypeDef IS31FL3741::action_on() {
    HAL_StatusTypeDef status = set_page(Page::k4);
    if (status != HAL_OK) {return status;}

    using namespace IS31FL3741_Config;
    status = i2c_.act_pollVerifyWrite(kAddr, rRegister, kDataOn);
    if (status != HAL_OK) {return status;}
...
HAL_StatusTypeDef IS31FL3741::action_reset_all_leds() {
    HAL_StatusTypeDef status = set_page(Page::k4);
    if (status != HAL_OK) {return status;}

    constexpr uint8_t rReset = 0x3F;
    constexpr uint8_t kReset = 0xAE;
    status = i2c_.act_pollWrite(kAddr, rReset, kReset);
    if (status != HAL_OK) {return status;}
    ...
    status = setup_pull_resistor();
    if (status != HAL_OK) {return status;}
    status = set_global_current(global_current_);
    if (status != HAL_OK) {return status;}
...
HAL_StatusTypeDef IS31FL3741::setup_config_register() {
    HAL_StatusTypeDef status = set_page(Page::k4);
    if (status != HAL_OK) {return status;}

    using namespace IS31FL3741_Config;
    return i2c_.act_pollVerifyWrite(kAddr, rRegister, kDataOff);
}

HAL_StatusTypeDef IS31FL3741::setup_pull_resistor() {
    HAL_StatusTypeDef status = set_page(Page::k4);
    if (status != HAL_OK) {return status;}
    ...
    return i2c_.act_pollVerifyWrite(kAddr, rPullResistor,
                                 kPullResistor);
}
...
HAL_StatusTypeDef IS31FL3741::set_page(const Page page) {
    ...
    HAL_StatusTypeDef status = i2c_.act_pollVerifyWrite(kAddr, rUnlock, kUnlock);
    if (status != HAL_OK) {return status;}

    constexpr uint8_t rPageSelect = 0xFD;
    return i2c_.act_pollWrite(kAddr, rPageSelect, 
                                 static_cast<uint8_t>(page));
}

HAL_StatusTypeDef IS31FL3741::action_write_page(const Page page,
                                        uint8_t* data,
    ...
    return i2c_.act_pollWriteMulti(kAddr, rInitialAddress, data,
                                   data_len);
}