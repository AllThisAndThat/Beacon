...
int main(void)
{
  ...
  const uint16_t dev_address = 0x29 << 1;
  const uint16_t mem_address = 0x86;
  uint8_t data = 0x0;
  HAL_StatusTypeDef status = HAL_OK;
  while (1)
  {
    data = 0;
    status = HAL_I2C_Mem_Read(&hi2c1, dev_address, mem_address, I2C_MEMADD_SIZE_8BIT, &data, sizeof(uint8_t), 1000);
    if (status != HAL_OK) {
